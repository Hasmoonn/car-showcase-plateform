            ...
            <div className='flex min-h-full items-center justify-center p-4 text-center'>
              <TransitionChild as={Fragment} enter='ease-out duration-300' enterFrom='opacity-0 scale-95' enterTo='opacity-100 scale-100' leave='ease-in duration-200' leaveFrom='opacity-100 scale-100' leaveTo='opacity-0 scale-95'>
                <DialogPanel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white text-left shadow-xl transition-all flex flex-col gap-5 p-6">
                  <button type='button' onClick={closeModel} className='absolute top-2 right-2 z-10 w-fit p-2 bg-primary-blue-100 rounded-full cursor-pointer'>
                    ...
                    <div className='mt-3 flex flex-wrap gap-4'>
                      {Object.entries(car).map(([key, value]) => (
                        <div className='flex justify-between gap-5 w-full' key={key}>
                          <h4 className='capitalize text-grey text-left'>{key.split("_").join(" ")}</h4>
                          <p className='text-black-100 font-semibold text-right'>{value}</p>
                        </div>
                      ))}
                    </div>
                  </div>
